...
# for(i in length(unique(us.birds$AOU))) {
#   a <- subset(us.birds, AOU == unique(us.birds$AOU[i]))
#   b <- aggregate(SpeciesTotal ~ Year, data=a, FUN=mean)
#   c <- subset(t.data.i, Year %in% b$Year)
#   d <- merge(c, b, by = 'Year')
# }
```
...
#### Conclusion: I began this project wanting to analyze the relationship between Lesser Yellowlegs and the increasing high temperatures. From this I found that there are not quite enough data points for the species to form a reliable conclusion about the correlation. However the idea of doing this type of analysis on any species within the dataset was intriguing to me, so I wrote a function to do just that. This function would be more useful on larger datasets, but it still demonstrates a simple way to analyze correlations between two variables. By looking at several correlations, I notice a pattern of consistantly positive correlations, which is not what I would expect to see. The issue with the north american breeding bird survey dataset is that the number of observations seems to increase overtime for all bird species, even when the actual population sizes have been found to be decreasing. This suggests that the survey is not an accurate measurement of population size and may be biased.