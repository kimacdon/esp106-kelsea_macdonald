...
### The only problem with this function now is if you do not know the species ID, so then wrote a function to look up the ID (AOU) given the common english name for the species.
```{r}
library("readxl")
...
names[c(1:17),2] <-  
  sub(".", "", names[c(1:17),2])
# names <- names[order(AOU, decreasing = TRUE),]
